...
流れを言語化すると、「JSX → JS → VDOMオブジェクト」と3段階で変化しているように見える。ただし各段階の性質は違う。

| 段階      | 何が起きるか                                                               | タイミング     |
| --------- | -------------------------------------------------------------------------- | -------------- |
| JSX → JS  | コンパイラがソースコードを書き換える                                       | **ビルド時**   |
| JS → VDOM | 実行されたJSが `createElement(...)` を呼び、戻り値としてオブジェクトが返る | **ランタイム** |

つまり後半は「変換」ではなく「**関数呼び出しの結果**」。コンパイラが生成するのはあくまで `createElement("h1", ...)` という**式**であって、それが実行されて初めてオブジェクトが生まれる。

この「ビルド時 / ランタイム」の区別は、後で Hooks や Reconciler を理解するときに効いてくる。Hooks はランタイム側の仕組みで、JSXの構文変換とは完全に独立した話。
